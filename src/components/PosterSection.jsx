 ...
 */
    const newAr = festivalData.filter((fests) => fests.date === date).sort((a, b) => a.importance - b.importance);
    // console.log(newAr)
  return (
      ...
      {newAr.map((fest)=><p className={fest.importance === 1  ? 'giant': fest.importance === 2? 'big': fest.importance === 3? 'medium': 'small'}>{fest.name}</p>)}
      </div>
    )
}
